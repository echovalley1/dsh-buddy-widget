...
function geomCore(bh) {
  try {
    ...
    return { w: w, h: h, bw: bw, bh: bh2, iconTop: iconTop, defTop: defTop, lowTop: lowTop }
  } catch (e) { return { w: 0, h: 0, bw: 0, bh: 0, iconTop: 0, defTop: 0, lowTop: 0 } }
}
function pageTopLocal() {
  try { return Math.max(0, root.getBoundingClientRect().top || 0) } catch (e) { return 0 }
}
function bubbleGeom() {
  try {
    var c = geomCore()
    ...
    var left = clamp(Math.round(c.w * 0.34) + Math.round(dx * c.w), 0, Math.max(0, c.w - c.bw))
    var lo = -Math.round(pageTopLocal())
...
function onBubbleDown(e) {
  ...
  bDrag = {
    ...
    x0: br ? (br.left - rr.left) : c.defTop,
    y0: br ? (br.top - rr.top) : 0,
    moved: false,
    sign: mirrored ? -1 : 1,
    c: c, pageTop: Math.max(0, rr.top || 0)
  }
...
function onBubbleMove(e) {
  ...
  var c = bDrag.c
  var nx = clamp(bDrag.x0 + dx, 0, Math.max(0, c.w - c.bw))
