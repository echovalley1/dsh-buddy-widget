...
var css = [
  ...
  '.dshb-bubble{position:absolute;left:33%;width:67%;bottom:calc(var(--dshb-u) * 72);display:flex;justify-content:center;pointer-events:none;opacity:0;transform:scale(.88) translateY(calc(var(--dshb-u) * 5));transform-origin:50% 100%;transition:opacity .18s ease,transform .2s cubic-bezier(.34,1.56,.64,1);z-index:2}',
...
// 水平滑轨，且气泡整体不得超出挂件盒子（挂件始终在页面内 → 气泡不会超出页面）。
// 默认锚点左 33%、宽 67% → 右缘恰贴挂件右缘；贴屏幕右缘时只能向左滑，永不越界。
var BUBBLE_DX_LO = -0.33 // 最左：气泡左缘 = 挂件左缘（不再越界）
var BUBBLE_DX_HI = 0 // 最右：默认已贴右缘
...
function bAnchor() {
  try {
    ...
    return { w: w, h: h, left: Math.round(w * 0.33), top: Math.max(0, Math.round(h - w * 0.72 - bh)), bh: bh }
