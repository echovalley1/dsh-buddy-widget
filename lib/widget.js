...
// 只允许水平滑移：尾巴中心停在图标上沿范围 [0.34w, w] 内，
// 最左不超过图标上沿的左上角（镜像停靠时几何对称，同样成立），纵向保持贴图标上沿。
var BUBBLE_DX_MAX = 0.32 // dx 上限：默认中心在 0.67w，±0.32w → 正好在图标上沿两端点内
...
function renderBubbleOff() {
  try {
    ...
    var a = bAnchor()
    if (!a.w) return
    bubble.style.left = (a.left + Math.round(clamp(bubbleOff.dx, -BUBBLE_DX_MAX, BUBBLE_DX_MAX) * a.w)) + 'px'
...
function saveBubbleOff() {
  try {
    if (bubbleOff) localStorage.setItem('dshb-bubble-off', JSON.stringify({ v: 2, dx: bubbleOff.dx }))
...
function loadBubbleOff() {
  try {
    ...
    if (a && isFinite(Number(a && a.dx))) {
      bubbleOff = { dx: clamp(Number(a.dx), -BUBBLE_DX_MAX, BUBBLE_DX_MAX) }
    }
  } catch (e) {}
}
...
function onBubbleMove(e) {
  ...
  // 只取水平分量，且不超过图标上沿两端（最左＝图标上沿左上角）
  bubbleOff = { dx: clamp(bDrag.dx + bDrag.sign * dx / bDrag.a.w, -BUBBLE_DX_MAX, BUBBLE_DX_MAX) }
  renderBubbleOff()
}
function onBubbleUp() {
  if (!bDrag) return
