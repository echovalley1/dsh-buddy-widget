...
function renderBubbleOff() {
  try {
    ...
    var a = bAnchor()
    if (!a.w) return
    bubble.style.left = (a.left + Math.round(bubbleOff.dx * a.w)) + 'px'
    bubble.style.top = (a.top + Math.round(bubbleOff.dy * a.h)) + 'px'
    bubble.style.bottom = 'auto'
  } catch (e) {}
}
function saveBubbleOff() {
  try {
    if (bubbleOff) localStorage.setItem('dshb-bubble-off', JSON.stringify({ v: 1, dx: bubbleOff.dx, dy: bubbleOff.dy }))
...
function loadBubbleOff() {
  try {
    ...
    if (a && a.v === 1 && isFinite(Number(a.dx)) && isFinite(Number(a.dy))) {
      bubbleOff = { dx: clamp(Number(a.dx), -0.8, 0.8), dy: clamp(Number(a.dy), -0.8, 0.8) }
    }
  } catch (e) {}
}
...
function onBubbleDown(e) {
  ...
  var a = bAnchor()
  if (!a.w) return
  bDrag = { x: e.clientX, y: e.clientY, dx: bubbleOff ? bubbleOff.dx : 0, dy: bubbleOff ? bubbleOff.dy : 0, moved: false, a: a }
...
function onBubbleMove(e) {
  ...
  bubbleOff = {
    dx: clamp(bDrag.dx + dx / bDrag.a.w, -0.8, 0.8),
    dy: clamp(bDrag.dy + dy / bDrag.a.h, -0.8, 0.8)
  }
  renderBubbleOff()
}
function onBubbleUp() {
