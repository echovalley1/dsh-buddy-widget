...
// ---------- 气泡沿图标上沿水平拖动（位置记忆；菜单可复位） ----------
// 默认锚点左 33.5%、宽 67% → 气泡中心 = 67%（图标上沿水平中点），复位即回到该中点；
// 拖拽只允许向左滑（镜像时方向已修正），气泡整体不出挂件盒子/页面。
var BUBBLE_DX_LO = -0.335 // 最左：气泡左缘 = 挂件左缘（不再越界）
var BUBBLE_DX_HI = 0 // 最右：默认即对准图标上沿中点
var bubbleOff = null // {dx} 相对默认锚点的水平位移（root 宽比例）
var bDrag = null
...
function bAnchor() {
  try {
    ...
    return { w: w, h: h, left: Math.round(w * 0.335), top: Math.max(0, Math.round(h - w * 0.72 - bh)), bh: bh }
  } catch (e) { return { w: 0, h: 0, left: 0, top: 0, bh: 0 } }
}
function renderBubbleOff() {
  try {
    if (!bubbleOff) { bubble.style.left = ''; bubble.style.top = ''; bubble.style.bottom = ''; return }
    var a = bAnchor()
    if (!a.w) return
    bubble.style.left = (a.left + Math.round(clamp(bubbleOff.dx, BUBBLE_DX_LO, BUBBLE_DX_HI) * a.w)) + 'px'
    bubble.style.top = a.top + 'px' // 纵向锁定在图标上沿
    bubble.style.bottom = 'auto'
  } catch (e) {}
}
function saveBubbleOff() {
  try {
    if (bubbleOff) localStorage.setItem('dshb-bubble-off', JSON.stringify({ v: 3, dx: bubbleOff.dx }))
...
function loadBubbleOff() {
  try {
    ...
    if (a && isFinite(Number(a && a.dx))) {
      bubbleOff = { dx: clamp(Number(a.dx), BUBBLE_DX_LO, BUBBLE_DX_HI) }
    }
  } catch (e) {}
}
...
function onBubbleDown(e) {
  ...
  bDrag = { x: e.clientX, y: e.clientY, dx: bubbleOff ? bubbleOff.dx : 0, moved: false, a: a, sign: mirrored ? -1 : 1 }
...
function onBubbleMove(e) {
  ...
  // 只取水平分量；气泡整体约束在挂件盒内（≈页面内，贴右缘时只能向左滑）
  bubbleOff = { dx: clamp(bDrag.dx + bDrag.sign * dx / bDrag.a.w, BUBBLE_DX_LO, BUBBLE_DX_HI) }
  renderBubbleOff()
}
function onBubbleUp() {
  if (!bDrag) return
