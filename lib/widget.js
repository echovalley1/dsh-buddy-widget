...
var css = [
  ...
  '.dshb-tail{position:absolute;left:50%;bottom:calc(var(--dshb-u) * -2 * var(--dshb-bub,1));width:calc(var(--dshb-u) * 8 * var(--dshb-bub,1));height:calc(var(--dshb-u) * 8 * var(--dshb-bub,1));margin-left:calc(var(--dshb-u) * -4 * var(--dshb-bub,1));background:#fff;border-right:calc(var(--dshb-u) * 2 * var(--dshb-bub,1)) solid #203170;border-bottom:calc(var(--dshb-u) * 2 * var(--dshb-bub,1)) solid #203170;border-bottom-right-radius:calc(var(--dshb-u) * 2 * var(--dshb-bub,1));transform:rotate(45deg);transform-origin:50% 50%;pointer-events:none;z-index:0}',
...
box.appendChild(txt)
bubble.appendChild(tailEl) // 尾巴垫在气泡盒下层：上部被盒体/描边盖住，衔接平滑
...
function geomCore(bh, rrIn) {
  try {
    ...
    var V = vp()
    // 以页面为可动边界（root 本地坐标：页面左缘=-rr.left，页面右缘=vpw-rr.left）
    var leftMin = rr ? -Math.round(Math.max(0, rr.left || 0)) : 0
    var leftMax = (V.w - (rr ? rr.left : 0)) - bw
    ...
    return {
      w: w, h: h, bw: bw, bh: bh2, iconTop: iconTop, defTop: defTop, lowTop: lowTop,
      leftMin: leftMin, leftMax: Math.max(leftMin, leftMax), pageTop: rr ? Math.max(0, rr.top || 0) : 0,
    }
  } catch (e) { return { w: 0, h: 0, bw: 0, bh: 0, iconTop: 0, defTop: 0, lowTop: 0, leftMin: 0, leftMax: 0, pageTop: 0 } }
}
...
function bubbleGeom() {
  try {
    ...
    var rr
    try { rr = root.getBoundingClientRect() } catch (e) {}
    var xb = pageXBounds(c, rr, state.h === 'left')
...
// 尾巴：在气泡底部中心，指向图标中心点（root 本地坐标角度；镜像随整体保持正确）
function updateTail() {
  try {
    if (!tailEl) return
    var g = bubbleGeom()
    if (!g.w || !g.bh) return
    var bx = g.left + g.bw / 2
    var by = g.top + g.bh
    var rad = Math.atan2(g.h * 0.67 - by, g.w * 0.67 - bx)
    tailEl.style.transform = 'rotate(' + (rad * 180 / Math.PI - 45) + 'deg)'
  } catch (e) {}
}
...
function onBubbleMove(e) {
  ...
  bubble.style.bottom = 'auto'
  // 尾巴实时指向图标中心点（复用缓存的尺寸，不触发布局）
  try {
    var bx = nx + c.bw / 2
    var by = ny + c.bh
    var rad = Math.atan2(c.h * 0.67 - by, c.w * 0.67 - bx)
    tailEl.style.transform = 'rotate(' + (rad * 180 / Math.PI - 45) + 'deg)'
  } catch (e) {}
}
function onBubbleUp() {
