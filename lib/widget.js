...
var css = [
  ...
  '.dshb-bubble{position:absolute;left:33.5%;width:67%;bottom:calc(var(--dshb-u) * 72);display:flex;justify-content:center;pointer-events:none;opacity:0;transform:scale(.88) translateY(calc(var(--dshb-u) * 5));transform-origin:50% 100%;transition:opacity .18s ease,transform .2s cubic-bezier(.34,1.56,.64,1);z-index:2}',
...
// ---------- 气泡自由移动（限制在图标上方不重叠区域；尾巴始终指向图标中心点） ----------
// 允许气泡在“图标正上方”区域内自由平移：气泡任何部分不遮挡图标，图标也不遮挡气泡。
// 尾巴（dshb-tail）始终从气泡底部中心指向图标中心点。
var bubbleOff = null // {dx,dy} 相对默认锚点的位移（root 宽/高的比例）
var bDrag = null
var blockBubbleClick = false
var BUBBLE_MARGIN = 0.06 // 气泡下缘到图标上沿的最小空隙（root 高比例；与 CSS bottom:72u 一致）
function bubbleGeom() {
  try {
    var w = root.offsetWidth || 0
    var h = root.offsetHeight || 0
    var bw = Math.round(w * 0.67)
    var bh = bubble.offsetHeight || 0
    var iconTop = Math.round(h * 0.34) // 图标顶部（root 本地坐标 y）
    var bottomMax = Math.max(0, iconTop - Math.round(h * BUBBLE_MARGIN)) // 气泡下缘允许的最大 y（<=图标上沿-空隙 → 永不压图标）
    var defTop = bottomMax - bh // 默认顶边（气泡高时可为负，向挂件顶部之上延伸，仍不压图标）
    ...
    var left = clamp(Math.round(w * 0.335) + Math.round(dx * w), 0, Math.max(0, w - bw))
    // 纵向可自由上移，直到浏览器页面顶（root 之上区域仍可见；root 顶部在页面内的距离为 rootRect.top）
    var pageTop = 0
    try { pageTop = Math.max(0, root.getBoundingClientRect().top || 0) } catch (e) {}
    var topMin = Math.min(defTop, -Math.round(pageTop)) // 上界（可为负，不超过页面顶）
    var top = clamp(defTop + Math.round(dy * h), topMin, defTop)
    return { w: w, h: h, bw: bw, bh: bh, left: left, top: top }
  } catch (e) { return { w: 0, h: 0, bw: 0, bh: 0, left: 0, top: 0 } }
}
// 尾巴：在气泡底部中心，指向图标中心点（root 本地坐标下的角度；镜像时整体随镜像保持正确）
function updateTail() {
  try {
    if (!tailEl) return
    var g = bubbleGeom()
    ...
    var by = g.top + g.bh
    var cx = g.w * 0.67 // 图标中心（66% 方块右下角 → 中心 67%,67%）
    var cy = g.h * 0.67
    var rad = Math.atan2(cy - by, cx - bx)
    var deg = rad * 180 / Math.PI
    tailEl.style.transform = 'rotate(' + (deg - 45) + 'deg)'
  } catch (e) {}
}
...
function saveBubbleOff() {
  try {
    if (bubbleOff) localStorage.setItem('dshb-bubble-off', JSON.stringify({ v: 4, dx: bubbleOff.dx, dy: bubbleOff.dy }))
...
function onBubbleDown(e) {
  ...
  var g = bubbleGeom()
  if (!g.w || !g.bh) return
  // 镜像停靠（左吸附）时 root 带 scaleX(-1)，视觉水平方向反转，拖拽位移需取反
  var mirrored = state.h === 'left'
  bDrag = { x: e.clientX, y: e.clientY, dx: bubbleOff ? Number(bubbleOff.dx) : 0, dy: bubbleOff ? Number(bubbleOff.dy) : 0, moved: false, g: g, sign: mirrored ? -1 : 1 }
...
function onBubbleMove(e) {
  if (!bDrag) return
  var dx = e.clientX - bDrag.x
  var dy = e.clientY - bDrag.y
  if (dx * dx + dy * dy >= CLICK_SQ) bDrag.moved = true
  if (!bDrag.moved) return
  // 记录目标偏移（范围在 bubbleGeom/renderBubbleOff 内夹紧：不遮挡图标、不出挂件盒）
  bubbleOff = {
    dx: bDrag.dx + bDrag.sign * dx / bDrag.g.w,
    dy: bDrag.dy + dy / bDrag.g.h
  }
  renderBubbleOff()
}
function onBubbleUp() {
