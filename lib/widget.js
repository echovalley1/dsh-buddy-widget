...
var BUBBLE_MARGIN = 0.04 // 气泡下缘到图标上沿的最小空隙（root 高比例）
function bubbleGeom() {
  try {
    ...
    var bottomMax = Math.max(0, iconTop - Math.round(h * BUBBLE_MARGIN)) // 气泡下缘允许的最大 y
    var defTop = Math.max(-bh, bottomMax - bh) // 默认贴紧图标上方；过高时可向上溢出（仍不压图标）
    ...
    // 纵向范围：[defTop - 可上移量, defTop]，defTop<0 时保持不压图标（可向 widget 顶部之上溢出）
    var freeUp = defTop >= 0 ? defTop : 0
    var top = clamp(defTop + Math.round(dy * h), defTop - freeUp, defTop)
