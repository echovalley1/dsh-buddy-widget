...
var PET_DONE_URL = '/dsh-buddy/pet?slot=done' // 完成庆祝图标（done）
var TOKEN_URL = '/dsh-buddy/platform-token' // PUT 写入 / DELETE 移除 DEEPSEEK_PLATFORM_TOKEN
var IDLE_EMOJI = '\ud83d\udc19' // 🐙
var BUSY_EMOJI = '\u270d\ufe0f' // ✍️（回复默认，未设回复图标时使用）
var DONE_EMOJI = '\ud83c\udf89' // 🎉（完成庆祝默认，未设完成图标时使用）
...
var css = [
  ...
  '.dshb-bubble{position:absolute;left:34%;width:66%;bottom:calc(var(--dshb-u) * 80);display:flex;justify-content:center;pointer-events:none;opacity:0;transform:scale(.88) translateY(calc(var(--dshb-u) * 5));transform-origin:50% 100%;transition:opacity .18s ease,transform .2s cubic-bezier(.34,1.56,.64,1);z-index:2}',
  '.dshb-bubble.dshb-open{opacity:1;transform:none;pointer-events:auto;cursor:pointer}',
  '.dshb-box{position:relative;max-width:100%;background:#fff;border:calc(var(--dshb-u) * 2 * var(--dshb-bub,1)) solid #203170;border-radius:calc(var(--dshb-u) * 12 * var(--dshb-bub,1));box-shadow:0 calc(var(--dshb-u) * 4 * var(--dshb-bub,1)) calc(var(--dshb-u) * 12 * var(--dshb-bub,1)) rgba(32,49,112,.28);padding:calc(var(--dshb-u) * 5 * var(--dshb-bub,1)) calc(var(--dshb-u) * 12 * var(--dshb-bub,1));box-sizing:border-box;color-scheme:light}',
  '.dshb-box::after{content:"";position:absolute;left:50%;bottom:calc(var(--dshb-u) * -4 * var(--dshb-bub,1));width:calc(var(--dshb-u) * 7 * var(--dshb-bub,1));height:calc(var(--dshb-u) * 7 * var(--dshb-bub,1));margin-left:calc(var(--dshb-u) * -3.5 * var(--dshb-bub,1));background:#fff;border-right:calc(var(--dshb-u) * 2 * var(--dshb-bub,1)) solid #203170;border-bottom:calc(var(--dshb-u) * 2 * var(--dshb-bub,1)) solid #203170;transform:rotate(45deg);border-bottom-right-radius:calc(var(--dshb-u) * 2 * var(--dshb-bub,1))}',
  ...
  '.dshb-menu .dshb-token{flex:1 1 auto;min-width:0;border:1px solid rgba(32,49,112,.4);border-radius:6px;padding:3px 6px;font-size:12px;color:#203170;background:#fff;box-sizing:border-box}',
...
// 用量模式：记账（余额差，免令牌） ⇄ 实时·令牌（平台用量，需 DEEPSEEK_PLATFORM_TOKEN）
var usageSelect = document.createElement('select')
usageSelect.className = 'dshb-sound'
usageSelect.title = '\u4eca\u65e5\u5df2\u7528\u53d6\u6570\u53e3\u5f84\uff1a\u8bb0\u8d26=\u4f59\u989d\u5dee\uff1b\u4ee4\u724c=\u5e73\u53f0\u7528\u91cf'
;(function () {
  var o1 = document.createElement('option'); o1.value = 'ledger'; o1.textContent = '\u8bb0\u8d26(\u4f59\u989d\u5dee)'
  var o2 = document.createElement('option'); o2.value = 'token'; o2.textContent = '\u5b9e\u65f6\u00b7\u4ee4\u724c'
  usageSelect.appendChild(o1); usageSelect.appendChild(o2)
})()
usageSelect.addEventListener('change', function () { setUsageMode(usageSelect.value) })
var rUsage = mrow(); rUsage.appendChild(mlabel('\u7528\u91cf')); rUsage.appendChild(usageSelect)
// 平台令牌：菜单内粘贴 → 由宿主写入 DSH 凭据（ctx.credentials.set），无需手改 YAML
var tkBtn = document.createElement('button')
tkBtn.type = 'button'
tkBtn.className = 'dshb-btn'
tkBtn.textContent = '\u8bbe\u7f6e\u2026'
tkBtn.title = '\u7c98\u8d34\u5f00\u653e\u5e73\u53f0\u767b\u5f55 token\uff08= localStorage.userToken \u7684 value\uff0c64 \u4f4d\uff1b\u6574\u6bb5 JSON \u4e5f\u884c\uff09'
var tkClear = document.createElement('button')
tkClear.type = 'button'
tkClear.className = 'dshb-btn'
tkClear.textContent = '\u6e05\u9664'
tkClear.title = '\u4ece DSH \u51ed\u636e\u4e2d\u79fb\u9664 DEEPSEEK_PLATFORM_TOKEN'
var rToken = mrow(); rToken.appendChild(mlabel('\u5e73\u53f0\u4ee4\u724c'))
var actTk = mactions(); actTk.appendChild(tkBtn); actTk.appendChild(tkClear); rToken.appendChild(actTk)
var tkPanel = document.createElement('div')
tkPanel.style.display = 'none'
var tkInput = document.createElement('input')
tkInput.type = 'password'
tkInput.className = 'dshb-token'
tkInput.placeholder = '\u7c98\u8d34 token\u2026'
tkInput.autocomplete = 'off'
tkInput.spellcheck = false
var tkSave = document.createElement('button')
tkSave.type = 'button'
tkSave.className = 'dshb-btn'
tkSave.textContent = '\u4fdd\u5b58'
var tkCancel = document.createElement('button')
tkCancel.type = 'button'
tkCancel.className = 'dshb-btn'
tkCancel.textContent = '\u53d6\u6d88'
var tkRow = mrow(); var tkAct = mactions()
tkAct.appendChild(tkSave); tkAct.appendChild(tkCancel)
tkRow.appendChild(tkInput); tkRow.appendChild(tkAct)
var tkStatus = document.createElement('div')
tkStatus.className = 'dshb-menu-note'
tkPanel.appendChild(tkRow)
tkPanel.appendChild(tkStatus)
...
var rIconDone = makeIconRow('done', '\u5b8c\u6210\u56fe\u6807')
var note = document.createElement('div')
note.className = 'dshb-menu-note'
menuBox.appendChild(r1)
menuBox.appendChild(r5)
menuBox.appendChild(r6)
menuBox.appendChild(rUsage)
menuBox.appendChild(rToken)
menuBox.appendChild(tkPanel)
menuBox.appendChild(r2)
menuBox.appendChild(r3)
menuBox.appendChild(r4)
menuBox.appendChild(rIconBusy)
menuBox.appendChild(rIconDone)
// 气泡位置复位
var bubbleResetBtn = document.createElement('button')
bubbleResetBtn.type = 'button'
bubbleResetBtn.className = 'dshb-btn'
bubbleResetBtn.textContent = '\u590d\u4f4d'
bubbleResetBtn.title = '\u5c06\u6c14\u6ce1\u56de\u5230\u56fe\u6807\u4e0a\u65b9\u5c45\u4e2d'
bubbleResetBtn.addEventListener('click', function () { resetBubblePos() })
var rBubPos = mrow(); rBubPos.appendChild(mlabel('\u6c14\u6ce1\u4f4d\u7f6e')); var actB = mactions(); actB.appendChild(bubbleResetBtn); rBubPos.appendChild(actB)
menuBox.appendChild(rBubPos)
...
var showBal = true
var usageMode = 'ledger' // 'ledger' 记账(余额差) | 'token' 实时·令牌(平台用量)
var hasPlatformToken = false
...
// 今日「tokens」= 非缓存部分（新输入 + 输出；输出已含推理 token，不再叠加）；缓存命中单独统计
function todayTokens() {
  var t = state.today.tokens || {}
  return (Number(t.input) || 0) + (Number(t.output) || 0)
}
function todayCacheTokens() { return Number((state.today.tokens || {}).cache) || 0 }
function todayReasonTokens() { return Number((state.today.tokens || {}).reason) || 0 }
// 今日已用：按用户所选模式取宿主值（记账=余额差 / 令牌=平台用量），缺失时回退事件估算
function todayUsageAmount() {
  if (balance && balance.ok && isFinite(Number(balance.todayUsage))) return Number(balance.todayUsage)
  return todayCost()
}
function todayUsageLabel() {
  var mode = balance && balance.ok ? balance.usageMode : usageMode
  return mode === 'token' ? '\u4eca\u65e5\u5df2\u7528(\u4ee4\u724c)' : '\u4eca\u65e5\u5df2\u7528(\u8bb0\u8d26)'
}
...
function statRows() {
  var t = state.today || {}
  var tokenMode = !!(balance && balance.ok && balance.usageMode === 'token')
  rowA.textContent = todayUsageLabel()
  rowB.textContent = fmtMoney(todayUsageAmount(), 'CNY')
  rowB.classList.add('dshb-red')
  var parts = []
  // 令牌模式：token 数取平台口径；记账模式：取本地事件累计（非缓存 = 输入 + 输出）
  if (tokenMode && Number(balance.todayUsageTokens) > 0) {
    parts.push('tokens ' + fmtTokens(Number(balance.todayUsageTokens)))
  } else {
    parts.push('tokens ' + fmtTokens(todayTokens()))
    var cache = todayCacheTokens()
    if (cache > 0) parts.push('\u7f13\u5b58 ' + fmtTokens(cache))
    var reason = todayReasonTokens()
    if (reason > 0) parts.push('\u63a8\u7406 ' + fmtTokens(reason))
  }
  parts.push((t.turns || 0) + ' \u8f6e')
...
function popupRows(l) {
  ...
  rowB.classList.add('dshb-red')
  var parts = ['tokens ' + fmtTokens(l.tokens)]
  if (l.cacheTokens > 0) parts.push('\u7f13\u5b58 ' + fmtTokens(l.cacheTokens))
...
// ---------- 气泡自由移动（区域受限：不遮挡图标；尾巴为静态 ::after 居中向下箭头） ----------
// 几何：图标 66% 方块（右下角）→ 图标顶部 y=34%、中心 (67%,67%)。
// 气泡宽 66%（中心默认 67%）。默认停在图标上方 18% 空隙处；可动范围为：
//   横向 = 整个浏览器页面（气泡始终高于图标顶，不压图标；贴右缘时只剩左向余地）
//   纵向 = 下缘最小 6% 空隙 ~ 页面上缘。
var bubbleOff = null // {dx,dy} 相对默认锚点的位移（root 宽/高的比例）
var bDrag = null
var blockBubbleClick = false
var BUBBLE_MIN_GAP = 0.06 // 气泡下缘到图标上沿的最小空隙（再低会压图标，禁止）
var BUBBLE_DEF_GAP = 0.18 // 复位/默认空隙（上下左右留足余量 → 拖动方向不“顶死”）
function geomCore(bh, rrIn) {
  try {
    var w = root.offsetWidth || 0
    var h = root.offsetHeight || 0
    var bw = Math.round(w * 0.66)
    var bh2 = Number.isFinite(Number(bh)) ? Number(bh) : (bubble.offsetHeight || 0)
    var rr = rrIn
    if (!rr) { try { rr = root.getBoundingClientRect() } catch (e) {} }
    var iconTop = Math.round(h * 0.34)
    var defTop = (iconTop - Math.round(h * BUBBLE_DEF_GAP)) - bh2
    var lowTop = (iconTop - Math.round(h * BUBBLE_MIN_GAP)) - bh2
    return {
      w: w, h: h, bw: bw, bh: bh2, rr: rr, defTop: defTop, lowTop: lowTop,
      pageTop: rr ? Math.max(0, rr.top || 0) : 0,
    }
  } catch (e) { return { w: 0, h: 0, bw: 0, bh: 0, rr: null, defTop: 0, lowTop: 0, pageTop: 0 } }
}
// 布局坐标下的横向可动范围（root 本地坐标）。镜像停靠视觉翻转，映射到布局坐标时范围取反。
function pageXBounds(c, rr, mirrored) {
  var V = vp()
  var L = Math.round(rr ? (rr.left || 0) : 0)
  if (mirrored) return { min: L + c.w - V.w, max: L + c.w - c.bw }
  return { min: -L, max: V.w - L - c.bw }
}
function bubbleGeom() {
  try {
    var c = geomCore()
    if (!c.w) return { w: 0, h: 0, bw: 0, bh: 0, left: 0, top: 0 }
    var dx = bubbleOff && isFinite(Number(bubbleOff.dx)) ? Number(bubbleOff.dx) : 0
    var dy = bubbleOff && isFinite(Number(bubbleOff.dy)) ? Number(bubbleOff.dy) : 0
    var xb = pageXBounds(c, c.rr, state.h === 'left') // 复用 geomCore 已量取的 rect，不再重复测量
    var left = clamp(Math.round(c.w * 0.34) + Math.round(dx * c.w), xb.min, xb.max)
    var lo = -Math.round(c.pageTop)
    var top = lo > c.lowTop ? c.lowTop : clamp(c.defTop + Math.round(dy * c.h), lo, c.lowTop)
    return { w: c.w, h: c.h, bw: c.bw, bh: c.bh, left: left, top: top }
  } catch (e) { return { w: 0, h: 0, bw: 0, bh: 0, left: 0, top: 0 } }
}
function renderBubbleOff() {
  try {
    if (!bubbleOff) { bubble.style.left = ''; bubble.style.top = ''; bubble.style.bottom = '' }
    else {
      var g = bubbleGeom()
      if (!g.w) return
      bubble.style.left = g.left + 'px'
      bubble.style.top = g.top + 'px'
      bubble.style.bottom = 'auto'
    }
  } catch (e) {}
}
function saveBubbleOff() {
  try {
    if (bubbleOff) localStorage.setItem('dshb-bubble-off', JSON.stringify({ v: 5, dx: bubbleOff.dx, dy: bubbleOff.dy }))
    else localStorage.removeItem('dshb-bubble-off')
  } catch (e) {}
}
function loadBubbleOff() {
  try {
    var a = JSON.parse(localStorage.getItem('dshb-bubble-off') || 'null')
    if (a && isFinite(Number(a && a.dx)) && isFinite(Number(a && a.dy))) {
      bubbleOff = { dx: Number(a.dx), dy: Number(a.dy) }
    }
  } catch (e) {}
}
function resetBubblePos() {
  bubbleOff = null
  bDrag = null
  blockBubbleClick = false
  saveBubbleOff()
  if (bubbleOpen) renderBubbleOff()
}
function setMode(m) {
...
function onState(d) {
  ...
  if (!seqAligned) {
    ...
    if (d.last && d.last.amount !== null && d.last.amount !== undefined) {
      ...
      celebrateDone() // 回复结束 → 完成庆祝图标播放约 3 遍
    }
  }
...
bubble.addEventListener('click', function (e) {
  ...
  if (blockBubbleClick) { blockBubbleClick = false; return } // 拖动后的 click 不关闭
  hideBubble()
})
// 气泡拖动：按下快照一次几何，拖动全程直接按鼠标像素差写入（跟手），仅到边界才夹紧
bubble.addEventListener('pointerdown', onBubbleDown, true)
function onBubbleDown(e) {
  if (!bubbleOpen) return
  if (e.button !== 0 && e.pointerType === 'mouse') return
  try { e.preventDefault(); e.stopPropagation() } catch (err) {}
  var rr
  try { rr = root.getBoundingClientRect() } catch (err) {}
  if (!rr) return
  var br
  try { br = bubble.getBoundingClientRect() } catch (err) {}
  var c = geomCore(br ? br.height : 0, rr)
  if (!c.w) return
  var mirrored = state.h === 'left' // 镜像停靠：root scaleX(-1)，水平位移需取反
  // 拖动期间禁用一切 CSS 过渡，保证速度 1:1 跟手
  bubble.style.transition = 'none'
  // pointer capture：快速甩出气泡/移出气泡后仍持续收事件，不丢速度
  try { if (bubble.setPointerCapture && e.pointerId !== undefined && e.pointerId !== null) bubble.setPointerCapture(e.pointerId) } catch (err) {}
  var xb = pageXBounds(c, rr, mirrored)
  var lx = bubble.offsetLeft || 0
  var ly = bubble.offsetTop || 0
  bDrag = {
    x: e.clientX, y: e.clientY,
    x0: lx, y0: ly,
    moved: false,
    sign: mirrored ? -1 : 1,
    c: c, xMin: xb.min, xMax: xb.max, pageTop: Math.max(0, rr.top || 0)
  }
  document.addEventListener('pointermove', onBubbleMove, true)
  document.addEventListener('pointerup', onBubbleUp, true)
  document.addEventListener('pointercancel', onBubbleCancel, true)
}
function onBubbleMove(e) {
  if (!bDrag) return
  // 位移始终以“按下点”为基准，单次事件只应用一次（勿在事件里改写 x0/y0，否则会逐帧重复累加 → 飞走）
  var dx = (e.clientX - bDrag.x) * bDrag.sign
  var dy = e.clientY - bDrag.y
  if (!bDrag.moved && dx * dx + dy * dy >= CLICK_SQ) bDrag.moved = true
  if (!bDrag.moved) return
  var c = bDrag.c
  var nx = clamp(bDrag.x0 + dx, bDrag.xMin, bDrag.xMax)
  var lo = -Math.round(bDrag.pageTop)
  var hi = c.lowTop
  var ny = lo > hi ? hi : clamp(bDrag.y0 + dy, lo, hi)
  bubble.style.left = Math.round(nx) + 'px'
  bubble.style.top = Math.round(ny) + 'px'
  bubble.style.bottom = 'auto'
}
function onBubbleUp() {
  if (!bDrag) return
  var moved = bDrag.moved
  var c = bDrag.c
  var x0 = bDrag.x0
  var y0 = bDrag.y0
  bDrag = null
  document.removeEventListener('pointermove', onBubbleMove, true)
  document.removeEventListener('pointerup', onBubbleUp, true)
  document.removeEventListener('pointercancel', onBubbleCancel, true)
  try { bubble.style.transition = '' } catch (err) {} // 恢复过渡（关闭动画用）
  if (moved) {
    blockBubbleClick = true // 抑制随后的 click（否则会误关气泡）
    setTimeout(function () { blockBubbleClick = false }, 400)
    bubbleOff = {
      dx: (x0 - c.w * 0.34) / c.w,
      dy: (y0 - c.defTop) / c.h
    }
...
function onBubbleCancel() { if (bDrag) bDrag.moved = false; onBubbleUp() }
...
// 切换用量模式：立即持久化 + 立刻拉一次余额（宿主会按新模式返回 todayUsage）
...
// 令牌模式但没令牌 → 菜单底部提示（写入/清除令牌后同步刷新）
function refreshUsageNote() {
  if (usageMode === 'token' && !hasPlatformToken) {
    note.textContent = '\u672a\u914d\u7f6e DEEPSEEK_PLATFORM_TOKEN\uff0c\u4ee4\u724c\u6a21\u5f0f\u5c06\u81ea\u52a8\u56de\u843d\u8bb0\u8d26\uff1b\u5728\u4e0b\u65b9\u300c\u5e73\u53f0\u4ee4\u724c\u300d\u7c98\u8d34\u540e\u5373\u65f6\u751f\u6548\u3002'
  } else if (note.textContent.indexOf('DEEPSEEK_PLATFORM_TOKEN') !== -1) {
    note.textContent = ''
  }
}
// 平台令牌：只写不读（宿主不回显值），成功/失败都只用状态文案反馈
function tkOpen(open) {
  tkPanel.style.display = open ? '' : 'none'
  tkStatus.textContent = ''
  if (open) {
    tkInput.value = ''
    try { tkInput.focus() } catch (err) {}
  }
}
function tkDone(label, ok) {
  tkStatus.textContent = label
  if (!ok) return
  try { tkInput.value = '' } catch (err) {}
  refreshUsageNote()
  balance = null
  pollBalance()
  if (!bubbleOpen || bubbleKind === 'stats') statRows()
}
function savePlatformToken(value) {
  var t = String(value || '').trim()
  if (!t) { tkStatus.textContent = '\u8bf7\u5148\u7c98\u8d34 token'; return }
  tkStatus.textContent = '\u4fdd\u5b58\u4e2d\u2026'
  try {
    fetch(TOKEN_URL, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ token: t }) })
      .then(function (r) { return r.json() })
      .then(function (d) {
        if (d && d.ok) {
          hasPlatformToken = true
          tkDone('\u5df2\u4fdd\u5b58\u5230 DSH \u51ed\u636e\uff0c\u7acb\u5373\u751f\u6548', true)
        } else {
          tkStatus.textContent = '\u4fdd\u5b58\u5931\u8d25\uff1a' + ((d && d.error) || '\u672a\u77e5\u9519\u8bef')
        }
      })
      .catch(function () { tkStatus.textContent = '\u4fdd\u5b58\u5931\u8d25\uff1a\u8bf7\u6c42\u5f02\u5e38' })
  } catch (err) {
    tkStatus.textContent = '\u4fdd\u5b58\u5931\u8d25'
  }
}
function clearPlatformToken() {
  tkStatus.textContent = '\u6e05\u9664\u4e2d\u2026'
  try {
    fetch(TOKEN_URL, { method: 'DELETE' })
      .then(function (r) { return r.json() })
      .then(function (d) {
        if (d && d.ok) {
          hasPlatformToken = false
          tkDone('\u5df2\u4ece DSH \u51ed\u636e\u4e2d\u79fb\u9664', true)
        } else {
          tkStatus.textContent = '\u6e05\u9664\u5931\u8d25\uff1a' + ((d && d.error) || '\u672a\u77e5\u9519\u8bef')
        }
      })
      .catch(function () { tkStatus.textContent = '\u6e05\u9664\u5931\u8d25\uff1a\u8bf7\u6c42\u5f02\u5e38' })
  } catch (err) {
    tkStatus.textContent = '\u6e05\u9664\u5931\u8d25'
  }
}
tkBtn.addEventListener('click', function () { tkOpen(tkPanel.style.display === 'none') })
tkCancel.addEventListener('click', function () { tkOpen(false) })
tkSave.addEventListener('click', function () { savePlatformToken(tkInput.value) })
tkClear.addEventListener('click', function () { clearPlatformToken() })
tkInput.addEventListener('keydown', function (e) {
  if (e.key === 'Enter') { e.preventDefault(); savePlatformToken(tkInput.value) } else if (e.key === 'Escape') { tkOpen(false) }
})
...
function saveConfig() {
  try {
    fetch(CONFIG_URL, { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ scale: state.scale, showBalance: showBal, usageMode: usageMode, fontScale: fontScale, bubbleScale: bubbleScale }) })
  } catch (err) {}
}
function savePos() {
  try {
...
// —— 宠物图标：普通/回复/完成三槽，各分「自定义 + 打包默认」——
var idleCustom = false, idleCustomTs = 0
var busyCustom = false, busyCustomTs = 0
var doneCustom = false, doneCustomTs = 0
var petDefaultIdle = false // 插件打包默认图标
var petDefaultBusy = false
var petDefaultDone = false
var celebrating = false // 完成庆祝播放中（done 图标）
var celebrateTimer = null
var petDoneMs = 2600 // 完成图标播放时长（约 3 遍）
function petSlotUrl(slot) {
  if (slot === 'busy') return PET_BUSY_URL
  if (slot === 'done') return PET_DONE_URL
  return PET_URL
}
var lastPetSrc = null // 记录当前 img src，src 不变时不做重设（避免 GIF 重播）
function refreshIcon() {
  var useDone = celebrating && (doneCustom || petDefaultDone)
  var b = !useDone && mode === 'busy'
  var slot = useDone ? 'done' : (b ? 'busy' : 'idle')
  var hasCustom = slot === 'done' ? doneCustom : (slot === 'busy' ? busyCustom : idleCustom)
  var defPresent = slot === 'done' ? petDefaultDone : (slot === 'busy' ? petDefaultBusy : petDefaultIdle)
  var emo = slot === 'done' ? DONE_EMOJI : (slot === 'busy' ? BUSY_EMOJI : IDLE_EMOJI)
  if (hasCustom || defPresent) {
    var v = hasCustom ? ((slot === 'done' ? doneCustomTs : (slot === 'busy' ? busyCustomTs : idleCustomTs)) || Date.now()) : 'def0'
    var src = petSlotUrl(slot) + '?v=' + v
...
// 完成庆祝：显示完成图标约 3 遍（时长由宿主按 GIF 帧延时估算），播完回到常规图标
function celebrateDone() {
  if (celebrateTimer) { clearTimeout(celebrateTimer); celebrateTimer = null }
  if (!doneCustom && !petDefaultDone) return // 未配置完成图标则不庆祝（仅弹金额泡泡）
  celebrating = true
  refreshIcon()
  celebrateTimer = setTimeout(function () {
    ...
    celebrating = false
    refreshIcon()
  }, Math.max(1200, petDoneMs || 2600))
}
function setCustom(slot, has, ts) {
  if (slot === 'done') { doneCustom = has; doneCustomTs = has ? (Number(ts) || Date.now()) : 0 }
  else if (slot === 'busy') { busyCustom = has; busyCustomTs = has ? (Number(ts) || Date.now()) : 0 }
    ...
    fetch(petSlotUrl(slot), {
      ...
      .then(function (d) {
        if (d && d.ok) {
          ...
          note.textContent = (slot === 'busy' ? '\u5df2\u66ff\u6362\u56de\u590d\u56fe\u6807' : (slot === 'done' ? '\u5df2\u66ff\u6362\u5b8c\u6210\u56fe\u6807' : '\u5df2\u66ff\u6362\u56fe\u6807')) + ' (' + (d.mime || '') + ')'
        } else {
  ...
  fetch(petSlotUrl(slot), { method: 'DELETE' })
    ...
    .then(function (d) {
      if (d && d.ok) {
        ...
        note.textContent = slot === 'busy' ? '\u5df2\u6062\u590d\u9ed8\u8ba4\u56de\u590d\u56fe\u6807' : (slot === 'done' ? '\u5df2\u6062\u590d\u9ed8\u8ba4\u5b8c\u6210\u56fe\u6807' : '\u5df2\u6062\u590d\u9ed8\u8ba4\u56fe\u6807')
      } else {
...
function boot(d) {
  if (d) {
    ...
    // 用量口径：ledger=余额差（默认） / token=平台实时用量
    if (typeof d.hasPlatformToken === 'boolean') hasPlatformToken = d.hasPlatformToken
    if (d.usageMode === 'token' || d.usageMode === 'ledger') {
    ...
    if (typeof d.hasDefaultDonePet === 'boolean') petDefaultDone = d.hasDefaultDonePet
    if (d.hasPet) setCustom('idle', true, d.petTs || 0)
    if (d.hasBusyPet) setCustom('busy', true, d.petBusyTs || 0)
    if (d.hasDonePet) setCustom('done', true, d.petDoneTs || 0)
    if (typeof d.petDoneMs === 'number' && d.petDoneMs > 0) petDoneMs = d.petDoneMs
    if (d.hasKey === false) {
