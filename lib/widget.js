...
var r3 = mrow(); r3.appendChild(mlabel('\u4f4d\u7f6e')); r3.appendChild(resetBtn)
...
// —— 自定义宠物图标：idle 普通图标 / busy 回复图标，busy 时自动切换 ——
...
function refreshIcon() {
  var b = mode === 'busy'
  var has = b ? busyCustom : idleCustom
  if (has) {
    var v = (b ? busyCustomTs : idleCustomTs) || Date.now()
    emoji.style.display = 'none'
    petImg.style.display = 'block'
    petImg.src = petSlotUrl(b ? 'busy' : 'idle') + '?v=' + v
...
function boot(d) {
  if (d) {
    ...
    // 自定义图标：普通图标 + 回复图标（busy 状态自动切换，gif 浏览器原生播放）
